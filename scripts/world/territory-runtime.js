function worldAreaDate(a){return (a.approx?wl(['ок. ','c. ','болж. ']):'')+Math.max(610,a.from)+'–'+Math.min(1299,a.to)+' '+wt('era');}
...
function renderWorldTerritories(){
 ...
 for(const a of areas){
  const entry=WORLD_HISTORY.entries.find(e=>e.id===a.entry),selected=a.entry===worldState.area;
  ...
  const path=sn('path',{d:polygonPath(a.polygons),fill:a.color,stroke:a.color,role:'button',tabindex:0,'aria-label':wl(a.name)+' · '+wt(a.kind)+' · '+worldAreaDate(a)});
  ...
  path.onpointerdown=e=>{if(e.button!==0)return;e.stopPropagation();hideTooltip();pointer={id:e.pointerId,x:e.clientX,y:e.clientY,tx:mapState.tx,ty:mapState.ty,moved:false};path.setPointerCapture(e.pointerId);};
  ...
  const xs=ring.map(p=>p[0]),ys=ring.map(p=>p[1]),lon=(Math.min(...xs)+Math.max(...xs))/2,lat=(Math.min(...ys)+Math.max(...ys))/2,[x,y]=project([lon,lat]);
  const guide=sn('line',{class:'world-territory-guide','data-guide':a.entry,x1:x,y1:y,x2:x,y2:y});worldTerritoryLabels.append(guide);
  const text=sn('text',{x,y,class:'world-territory-label'+(selected?' selected':''),'text-anchor':'middle','data-entry':a.entry,'data-kind':a.kind,'data-width':(Math.max(...xs)-Math.min(...xs))/360*SVG_W});text.textContent=wl(a.name).split(' · ')[0];worldTerritoryLabels.append(text);
 }
}
function updateWorldTerritoryLabels(){
 ...
 for(const label of labels){
  ...
  label.style.display='';
  const width=Math.max(label.getComputedTextLength()*scale,label.textContent.length*font*.48);
