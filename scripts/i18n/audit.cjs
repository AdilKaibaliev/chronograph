...
function element(id){if(!els.has(id))els.set(id,new Proxy({classList:{add(){},remove(){},toggle(){},contains(){return false}},focus(){},querySelector(){return null},querySelectorAll(){return []},addEventListener(){},setAttribute(k,v){add(v)}},{set(o,k,v){if(k==='innerHTML'||k==='textContent')add(v);o[k]=v;return true}}));return els.get(id);}
