...
const latePeople=[...require('../late/biographies.cjs'),...require('../early/biographies.cjs'),...require('../to1789/biographies.cjs')];
...
rows.push(['Формирование государства','Formation of a state','Мамлекеттин калыптанышы']);
rows.push(['Географический ориентир','Geographical reference','Географиялык багыт'],['Османская столица','Ottoman capital','Осмон борбору']);
...
const biographies=[...require('../biographies.cjs'),...latePeople.map(p=>[p.name[0],p.article,...p.text])];
...
for(const p of latePeople){stars.push({name:p.name[0],birth:p.birth,death:p.death,era:p.era,cat:'scholar',group:'Позднейшие ученые суннитской традиции',...(p.approxBirth?{birthApprox:true}:{})});rows.push(p.name);}
...
let html=require('../late/prepare.cjs')(source).replaceAll('CHRONOGRAPH 1.0 RC3.8','CHRONOGRAPH 1.7.1');
...
rows.push(['Государства и общества мира','States and societies of the world','Дүйнөнүн мамлекеттери жана коомдору'],['610–1789 · выберите год или исторический рубеж','610–1789 · choose a year or historical milestone','610–1789 · жылды же тарыхый окуяны тандаңыз']);
const oldMethod='«Хронография» синхронизирует исламскую историю с политической историей Евразии 610–1789 годов. Современные границы используются только как географический ориентир. Исторические области не следует понимать как современные юридически демаркированные границы.';
const newMethod=['«Хронограф» показывает историю мира 610–1789 годов. Современные границы служат географическими ориентирами. Исторические контуры отображают приблизительные владения, области влияния и расселения для указанного периода.','Chronograph presents world history from 610 to 1789. Modern borders serve as geographical references. Historical outlines represent approximate domains, spheres of influence and settlement for the stated period.','«Хронограф» 610–1789-жылдардагы дүйнө тарыхын көрсөтөт. Азыркы чек аралар географиялык багыт берет. Тарыхый контурлар көрсөтүлгөн мезгилдеги болжолдуу ээликтерди, таасир жана конуш аймактарын чагылдырат.'];
