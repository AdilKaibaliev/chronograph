...
assert.equal(people.length,57);assert.equal(new Set(people.map(p=>p.name)).size,57);assert.equal(Object.keys(details).length,57);
for(const p of people){
 ...
 ctx.year=Math.min(1299,Math.max(610,(p.birth??p.knownBy)+25));box.scrollTop=200;openStar(p);
...
console.log('PASS: 57 individual biographies and plain-text sources in 3 languages; all contemporary buttons, dates, scroll reset and focus transitions.');
